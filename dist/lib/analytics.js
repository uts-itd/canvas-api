...
var analytics = {
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelParticipationDataTerms: function (accountId, termId) { return helper.get("/v1/accounts/" + accountId + "/analytics/terms/" + termId + "/activity"); },
    /**
     * Returns page view hits summed across all courses in the department. Two groupings of these counts are returned; one by day (+by_date+), the other by category (+by_category+). The possible categories are announcements, assignments, collaborations, conferences, discussions, files, general, grades, groups, modules, other, pages, and quizzes. This and the other department-level endpoints have three variations which all return the same style of data but for different subsets of courses. All share the prefix /api/v1/accounts/<account_id>/analytics. The possible suffixes are: /current: includes all available courses in the default term /completed: includes all concluded courses in the default term /terms/<term_id>: includes all available or concluded courses in the  given term. Courses not yet offered or which have been deleted are never included. /current and /completed are intended for use when the account has only one term. /terms/<term_id> is intended for use when the account has multiple terms. The action follows the suffix.
     * @summary Get department-level participation data
     * @param {string} accountId Canvas Account ID
     * @returns {Promise<any>}
     */
    getDepartmentLevelParticipationDataCurrent: function (accountId) { return helper.get("/v1/accounts/" + accountId + "/analytics/current/activity"); },
    /**
     * Returns page view hits summed across all courses in the department. Two groupings of these counts are returned; one by day (+by_date+), the other by category (+by_category+). The possible categories are announcements, assignments, collaborations, conferences, discussions, files, general, grades, groups, modules, other, pages, and quizzes. This and the other department-level endpoints have three variations which all return the same style of data but for different subsets of courses. All share the prefix /api/v1/accounts/<account_id>/analytics. The possible suffixes are: /current: includes all available courses in the default term /completed: includes all concluded courses in the default term /terms/<term_id>: includes all available or concluded courses in the  given term. Courses not yet offered or which have been deleted are never included. /current and /completed are intended for use when the account has only one term. /terms/<term_id> is intended for use when the account has multiple terms. The action follows the suffix.
     * @summary Get department-level participation data
     * @param {string} accountId Canvas Account ID
     * @returns {Promise<any>}
     */
    getDepartmentLevelParticipationDataCompleted: function (accountId) { return helper.get("/v1/accounts/" + accountId + "/analytics/completed/activity"); },
    /**
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelGradeDataTerms: function (accountId, termId) { return helper.get("/v1/accounts/" + accountId + "/analytics/terms/" + termId + "/grades"); },
    /**
     * Returns the distribution of grades for students in courses in the department. Each data point is one student's current grade in one course; if a student is in multiple courses, he contributes one value per course, but if he's enrolled multiple times in the same course (e.g. a lecture section and a lab section), he only constributes on value for that course. Grades are binned to the nearest integer score; anomalous grades outside the 0 to 100 range are ignored. The raw counts are returned, not yet normalized by the total count. Shares the same variations on endpoint as the participation data.
     * @summary Get department-level grade data
     * @param {string} accountId Canvas Account ID
     * @returns {Promise<any>}
     */
    getDepartmentLevelGradeDataCurrent: function (accountId) { return helper.get("/v1/accounts/" + accountId + "/analytics/current/grades"); },
    /**
     * Returns the distribution of grades for students in courses in the department. Each data point is one student's current grade in one course; if a student is in multiple courses, he contributes one value per course, but if he's enrolled multiple times in the same course (e.g. a lecture section and a lab section), he only constributes on value for that course. Grades are binned to the nearest integer score; anomalous grades outside the 0 to 100 range are ignored. The raw counts are returned, not yet normalized by the total count. Shares the same variations on endpoint as the participation data.
     * @summary Get department-level grade data
     * @param {string} accountId Canvas Account ID
     * @returns {Promise<any>}
     */
    getDepartmentLevelGradeDataCompleted: function (accountId) { return helper.get("/v1/accounts/" + accountId + "/analytics/completed/grades"); },
    /**
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelStatisticsTerms: function (accountId, termId) { return helper.get("/v1/accounts/" + accountId + "/analytics/terms/" + termId + "/statistics"); },
    /**
     * Returns numeric statistics about the department and term (or filter). Shares the same variations on endpoint as the participation data.
     * @summary Get department-level statistics
     * @param {string} accountId Canvas Account ID
     * @returns {Promise<any>}
     */
    getDepartmentLevelStatisticsCurrent: function (accountId) { return helper.get("/v1/accounts/" + accountId + "/analytics/current/statistics"); },
    /**
     * Returns numeric statistics about the department and term (or filter). Shares the same variations on endpoint as the participation data.
     * @summary Get department-level statistics
     * @param {string} accountId Canvas Account ID
     * @returns {Promise<any>}
     */
    getDepartmentLevelStatisticsCompleted: function (accountId) { return helper.get("/v1/accounts/" + accountId + "/analytics/completed/statistics"); },
    /**
     ...
     * @returns {Promise<any>}
     */
    getCourseLevelParticipationData: function (courseId) { return helper.get("/v1/courses/" + courseId + "/analytics/activity"); },
    /**
     ...
     * @returns {Promise<any>}
     */
    getCourseLevelAssignmentData: function (courseId, query) {
        if (query === void 0) { query = ''; }
        return helper.get("/v1/courses/" + courseId + "/analytics/assignments", query);
    },
    /**
     ...
     * @returns {Promise<any>}
     */
    getCourseLevelStudentSummaryData: function (courseId, query) {
        if (query === void 0) { query = ''; }
        return helper.get("/v1/courses/" + courseId + "/analytics/student_summaries", query);
    },
    /**
     ...
     * @returns {Promise<any>}
     */
    getUserInACourseLevelParticipationData: function (courseId, studentId) { return helper.get("/v1/courses/" + courseId + "/analytics/users/" + studentId + "/activity"); },
    /**
     ...
     * @returns {Promise<any>}
     */
    getUserInACourseLevelAssignmentData: function (courseId, studentId) { return helper.get("/v1/courses/" + courseId + "/analytics/users/" + studentId + "/assignments"); },
    /**
