...
export default function (helper: Helper) {
  return {
     ...
     * @returns {Promise<any>}
     */
    submitAssignmentCourses: (
      courseId: string | number,
      assignmentId: string | number,
      body: any,
    ) => helper.post(`/v1/courses/${courseId}/assignments/${assignmentId}/submissions`, body),
    /**
     * Make a submission for an assignment. You must be enrolled as a student in the course/section to do this. All online turn-in submission types are supported in this API. However, there are a few things that are not yet supported: Files can be submitted based on a file ID of a user or group file. However, there is no API yet for listing the user and group files, or uploading new files via the API. A file upload API is coming soon. Media comments can be submitted, however, there is no API yet for creating a media comment to submit. Integration with Google Docs is not yet supported.
     * @summary Submit an assignment
     ...
     * @returns {Promise<any>}
     */
    submitAssignmentSections: (
      sectionId: string | number,
      assignmentId: string | number,
      body: any,
    ) => helper.post(`/v1/sections/${sectionId}/assignments/${assignmentId}/submissions`, body),
    /**
    ...
    listAssignmentSubmissionsCourses: (
      ...
      assignmentId: string | number,
      query?: any,
    ) => helper.get(`/v1/courses/${courseId}/assignments/${assignmentId}/submissions`, query),
    /**
     * A paginated list of all existing submissions for an assignment.
     * @summary List assignment submissions
     * @param {string|number} sectionId Canvas Section ID
    ...
    listAssignmentSubmissionsSections: (
      ...
      assignmentId: string | number,
      query?: any,
    ) => helper.get(`/v1/sections/${sectionId}/assignments/${assignmentId}/submissions`, query),
    /**
     ...
     * @returns {Promise<any>}
     */
    listSubmissionsForMultipleAssignmentsCourses: (courseId: string | number, query?: any) =>
     ...
     * @returns {Promise<any>}
     */
    listSubmissionsForMultipleAssignmentsSections: (sectionId: string | number, query?: any) =>
    ...
    gradeOrCommentOnSubmissionCourses: (
      ...
      helper.put(`/v1/courses/${courseId}/assignments/${assignmentId}/submissions/${userId}`, body),
    /**
     * Comment on and/or update the grading for a student's assignment submission. If any submission or rubric_assessment arguments are provided, the user must have permission to manage grades in the appropriate context (course or section).
     * @summary Grade or comment on a submission
     * @param {string|number} sectionId Canvas Section ID
     ...
     * @returns {Promise<any>}
     */
    listGradeableStudents: (courseId: string | number, assignmentId: string | number) =>
      helper.get(`/v1/courses/${courseId}/assignments/${assignmentId}/gradeable_students`),
    /**
     ...
     * @returns {Promise<any>}
     */
    listMultipleAssignmentsGradeableStudents: (courseId: string | number, query?: any) =>
      helper.get(`/v1/courses/${courseId}/assignments/gradeable_students`, query),
    /**
     ...
     * @returns {Promise<any>}
     */
    gradeOrCommentOnMultipleSubmissionsCoursesSubmissions: (courseId: string | number, body: any) =>
    ...
    markSubmissionAsReadCourses: (
      ...
      helper.put(`/v1/courses/${courseId}/assignments/${assignmentId}/submissions/${userId}/read`),
    /**
     * No request fields are necessary. On success, the response will be 204 No Content with an empty body.
     * @summary Mark submission as read
     * @param {string|number} sectionId Canvas Section ID
     ...
     * @returns {Promise<any>}
     */
    submissionSummaryCourses: (
      courseId: string | number,
      assignmentId: string | number,
      query?: any,
    ) =>
      helper.get(`/v1/courses/${courseId}/assignments/${assignmentId}/submission_summary`, query),
    /**
     * Returns the number of submissions for the given assignment based on gradeable students that fall into three categories: graded, ungraded, not submitted.
     * @summary Submission Summary
     * @param {string|number} sectionId Canvas Section ID
     ...
     * @returns {Promise<any>}
     */
    submissionSummarySections: (
      sectionId: string | number,
      assignmentId: string | number,
      query?: any,
    ) =>
      helper.get(`/v1/sections/${sectionId}/assignments/${assignmentId}/submission_summary`, query),
  };
}
