module.exports = (helper) => {
  return {
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelParticipationDataTerms: (accountId, termId) =>
      helper.get(
        `/v1/accounts/${accountId}/analytics/terms/${termId}/activity`,
      ),
    /**
     * Returns page view hits summed across all courses in the department. Two groupings of these counts are returned; one by day (+by_date+), the other by category (+by_category+). The possible categories are announcements, assignments, collaborations, conferences, discussions, files, general, grades, groups, modules, other, pages, and quizzes. This and the other department-level endpoints have three variations which all return the same style of data but for different subsets of courses. All share the prefix /api/v1/accounts/<account_id>/analytics. The possible suffixes are: /current: includes all available courses in the default term /completed: includes all concluded courses in the default term /terms/<term_id>: includes all available or concluded courses in the  given term. Courses not yet offered or which have been deleted are never included. /current and /completed are intended for use when the account has only one term. /terms/<term_id> is intended for use when the account has multiple terms. The action follows the suffix.
     * @summary Get department-level participation data
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelParticipationDataCurrent: (accountId) =>
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelParticipationDataCompleted: (accountId) =>
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelGradeDataTerms: (accountId, termId) =>
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelGradeDataCurrent: (accountId) =>
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelGradeDataCompleted: (accountId) =>
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelStatisticsTerms: (accountId, termId) =>
      helper.get(
        `/v1/accounts/${accountId}/analytics/terms/${termId}/statistics`,
      ),
    /**
     * Returns numeric statistics about the department and term (or filter). Shares the same variations on endpoint as the participation data.
     * @summary Get department-level statistics
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelStatisticsCurrent: (accountId) =>
     ...
     * @returns {Promise<any>}
     */
    getDepartmentLevelStatisticsCompleted: (accountId) =>
     ...
     * @returns {Promise<any>}
     */
    getCourseLevelParticipationData: (courseId) =>
     ...
     * @returns {Promise<any>}
     */
    getCourseLevelAssignmentData: (courseId, query = '') =>
     ...
     * @returns {Promise<any>}
     */
    getCourseLevelStudentSummaryData: (courseId, query = '') =>
     ...
     * @returns {Promise<any>}
     */
    getUserInACourseLevelParticipationData: (courseId, studentId) =>
      helper.get(
        `/v1/courses/${courseId}/analytics/users/${studentId}/activity`,
      ),
    /**
     ...
     * @returns {Promise<any>}
     */
    getUserInACourseLevelAssignmentData: (courseId, studentId) =>
      helper.get(
        `/v1/courses/${courseId}/analytics/users/${studentId}/assignments`,
      ),
    /**
     ...
     * @returns {Promise<any>}
     */
    getUserInACourseLevelMessagingData: (courseId, studentId) =>
      helper.get(
        `/v1/courses/${courseId}/analytics/users/${studentId}/communication`,
      ),
  };
};
